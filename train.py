...
def train(model,training_data_loader,loss_name,t_model_type,optimizer):
    model.train()
    loss_print = 0
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        if loss_name == 'PUEILoss':
            loss = PUEILoss(R1, R2, L1, im1, X1)
        elif loss_name == 'TMGLoss':
            loss = TMGLoss(R1, L1, im1, X1,t_model_type)
        elif loss_name == 'SUEILoss':
            loss = SUEILoss(R1, L1, im1, X1)
...
def checkpoint_psnr(epoch,model,model_out_path,loss_name,t_model_type):
    torch.save(model.state_dict(), model_out_path+"/best_psnr_"+loss_name+"_"+t_model_type+".pth")
    #print("Checkpoint saved to {}".format(model_out_path+"/best_psnr_"+loss_name+"_"+t_model_type+".pth"))
	
def checkpoint_ssim(epoch,model,model_out_path,loss_name,t_model_type):
    torch.save(model.state_dict(), model_out_path+"/best_ssim_"+loss_name+"_"+t_model_type+".pth")
    #print("Checkpoint saved to {}".format(model_out_path+"/best_ssim_"+loss_name+"_"+t_model_type+".pth"))

def checkpoint_lpips(epoch,model,model_out_path,loss_name,t_model_type):
    torch.save(model.state_dict(), model_out_path+"/best_lpips_"+loss_name+"_"+t_model_type+".pth")
    #print("Checkpoint saved to {}".format(model_out_path+"/best_lpips_"+loss_name+"_"+t_model_type+".pth"))
...
if __name__ == '__main__':
    ...
    if opt.model_name == "DCRetinex":
        print('===> Building model DCRetinex')
        model= DCRetinex().cuda()
    

    model_out_path = opt.save_folder+opt.model_name    
    ...
    score_best = 0
    writer = SummaryWriter(opt.log_dir+opt.model_name+'/'+opt.loss_name)
    best_psnr=0
    best_ssim=0
    best_lpips=2
    for epoch in range(opt.start_iter, opt.nEpochs + 1):
        train(model,training_data_loader,opt.loss_name,opt.t_model_type,optimizer)
        scheduler.step()
        ...
        if psnr > best_psnr:
            best_psnr = psnr
            checkpoint_psnr(epoch,model,model_out_path,opt.loss_name,opt.t_model_type)
        if ssim > best_ssim:
            best_ssim = ssim
            checkpoint_ssim(epoch,model,model_out_path,opt.loss_name,opt.t_model_type)
        if lpips < best_lpips:
            best_lpips = lpips
            checkpoint_lpips(epoch,model,model_out_path,opt.loss_name,opt.t_model_type)
